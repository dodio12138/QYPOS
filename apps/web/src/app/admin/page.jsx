...
function UsersView({ usersList, rolesList, onSaved }) {
  const [editingId, setEditingId] = useState(null); // user id being edited, or "new"
  const [resettingPinFor, setResettingPinFor] = useState(null); // user id whose PIN is being reset
  const [deletingId, setDeletingId] = useState(null); // user id being delete-confirmed
  ...
  async function save(event) {
    event.preventDefault();
    if (!form.name.trim() || !form.role_id) {
      setLoadError("请填写姓名并选择角色");
      return;
    }
    if (editingId === "new" && !form.pin.trim()) {
      setLoadError("请设置 PIN");
      return;
    }
    if (form.pin && form.pin !== form.pinConfirm) {
      setLoadError("两次输入的 PIN 不一致");
      return;
    }
    try {
  ...
  async function saveResetPin(event) {
    ...
    try {
      ...
      setLoadError(error.message || "重置 PIN 失败");
    }
  }
  ...
  // PIN fields shared between create/edit and reset-pin forms
        ...
        <label style={{ minWidth: 130 }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <input
              ...
              placeholder={required ? "设置数字 PIN" : "留空则不修改"}
              autoFocus
              style={{ width: 130 }}
            />
    ...
    <div className="panel" style={{ marginTop: 16 }}>
      ...
      {editingId === "new" && (
        <div style={{ marginBottom: 14, padding: 14, border: "1.5px solid var(--accent)", borderRadius: 10, background: "var(--lavender)" }}>
          ...
          <div style={{ marginTop: 12 }}>
            <PinFields required submitLabel="创建账户" />
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gap: 6 }}>
        {users.map((u) => {
          ...
          return (
            <div key={u.id} style={{
              border: `1.5px solid ${isEditing ? "var(--accent)" : isResettingPin ? "#facc15" : "var(--line)"}`,
              ...
              <div style={{
                ...
                cursor: isEditing || isResettingPin ? "default" : "pointer",
              }} onClick={() => {
                if (!isEditing && !isResettingPin && !isDeleting) openEdit(u);
              }}>
                {/* Avatar */}
                <div style={{
                  width: 34, height: 34, borderRadius: "50%",
                  background: u.active ? (ROLE_STYLES[role]?.bg || "#f3f4f6") : "#f3f4f6",
                  color: u.active ? (ROLE_STYLES[role]?.color || "var(--muted)") : "var(--muted)",
                ...
                <div style={{ display: "flex", gap: 2, flexShrink: 0 }} onClick={(e) => e.stopPropagation()}>
                  ...
                  {!isDeleting && (
                    <button type="button" onClick={() => setDeletingId(u.id)} style={{ padding: "6px", fontSize: 12, borderRadius: 6, background: "transparent", color: "#ef4444" }} title="删除账户"><Trash2 size={14} /></button>
                  )}
                </div>
              </div>
