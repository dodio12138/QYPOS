...
- 多种支付方式：现金、刷卡、扫码、其他，以及 **Dojo Go** 刷卡终端；支持同一订单按任意金额混合使用现金和银行卡支付
- 支持零元结账、管理员授权免单，以及现金超额实收（客人无需找零）
- 餐桌状态实时更新（WebSocket）
...
- **员工管理**：员工增删改查、角色权限（店主 vs 收银员精细化控制）、排班、实际考勤 OFF、休息时长与时薪；排班转化率/效益按当前已发生时段计算
- **设置中心**：税率、服务费、币种、打印机配置；敏感税务设置需当前账号与 PIN 二次确认
- **数据看板**：今日营业额、订单数、客单价、热销菜品多选下钻与合并趋势图
- **订单管理**：已结账订单支持管理员授权调低金额、待退款计算和追加备注
- **销售报表**：历史数据查询、星期几筛选、日期预设、分类构成、30/60 分钟时段图，以及可独立切换的累计单量/累计营业额曲线
- **会计导出**：中英双语 CSV 与 Excel，包含数据关系说明、支付对账、每日汇总和逐单账簿
...
后台 `/admin → 分析` 可按当前报表日期范围导出 CSV 或 Excel。两种格式使用同一套数据口径：只统计已结账订单，取消单、草稿单、进行中订单和分单父单不会计入。

Excel 文件包含四个工作表：

| 工作表 | 内容 |
|------|------|
| `说明与汇总 Summary` | 中英文口径、币种、税务模式、核心汇总和计算关系 |
| `支付对账 Payments` | 按支付方式/渠道汇总收款、找零、保留现金和结算额 |
| `每日汇总 Daily` | 每日订单、销售、税费、实收入账、退款及客单价 |
| `订单账簿 Orders` | 每笔已结账订单的时间、支付、税费、调整原因和备注 |
...
实收入账 = 收款金额 - 找零
订单结算额 = 实收入账 - 保留现金
待退款 = MAX(订单结算额 - 当前订单总额, 0)
对账差异 = 订单结算额 - 当前订单总额 - 待退款（正常应为 0）
```

CSV 使用 UTF-8 BOM，金额保持为可直接求和的数字列；Excel 提供冻结表头、列宽和金额格式，并突出非零待退款或对账差异。
...
│   │       │   └── ops.js            # 运维管理
│   │       └── services/      # 业务服务
│   │           ├── accounting-csv.js # 会计 CSV 数据结构与序列化
│   │           ├── accounting-xlsx.js # 多工作表会计 Excel
...
| `/orders/:id/complimentary` | POST | 管理员授权免单结账 | `adjust_discount` |
| `/orders/:id/amount-adjustment` | POST | 调整已结账订单金额并记录原因 | `adjust_discount` |
...
| `/reports/sales.csv` | GET | 中英双语会计 CSV | `export_reports` |
| `/reports/sales.xlsx` | GET | 四工作表会计 Excel | `export_reports` |
